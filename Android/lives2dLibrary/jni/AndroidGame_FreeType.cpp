...
public:
	...
	void buildSystemFont(const char* font, int fontsize)
	{
		...
		LOGI("buildSystemFont length:%d",length);
		//保存字体的大小;
		m_fontSize = fontsize;
		...
		if (m_FTFace)
		{
			LOGI("buildSystemFont m_FTFace Clear");
		...
		if(m_FTLibrary==NULL)
		{
			LOGI("buildSystemFont m_FTLibrary==NULL");
			return;
		}
		LOGI("buildSystemFont m_FTLibrary ok");
		
		if(m_fontBuffer==NULL)
		{
			LOGI("m_fontBuffer==NULL");
			return;
		}
		
		FT_Error error = FT_New_Memory_Face(m_FTLibrary, (const FT_Byte*)m_fontBuffer, length, 0, &m_FTFace);
		LOGI("buildSystemFont FT_New_Memory_Face error %d",error);
		if (error != 0)
		{
		...
		LOGI("buildSystemFont FT_New_Memory_Face ok");
	...
	bool getCharacter(wchar_t ch)
	{
			...
			m_character[ch].y1 == 0)
		{
			...
			LOGI("FT_Get_Char_Index %d",index);
			...
			LOGI("bitmap.width %d,bitmap.rows %d",bitmap.width,bitmap.rows);
...
JNIEXPORT void JNICALL Java_com_lives2d_library_nativeWrap_setSdCardPath(JNIEnv * env, jobject obj,jstring javaString)
{
	...
	std::string ttfpath=mSdCardPath+"/msyh.ttf";
	
	Font font;
	font.buildSystemFont(ttfpath.c_str(), 30);
